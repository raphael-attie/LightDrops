...
files = glob.glob("/Users/attie/Dropbox/USET/Data/HAlpha/*.FTS")[0:20]  # [0:80:4]
...
# radiusOffset    = 100
outdir = '/Users/attie/Dropbox/USET/Data/HAlpha/Python_lucky/'
...
for i in range(0, nImages / interval):
    ...
    newImage, shift, best_indices, xs, ys = li.lucky_imaging(images, globalRefImage, blk_size, nbest, binning)

    # Export the canvas to fits files. Rount to integers. Float is useless here.
    rImage = np.rint(newImage)
    intImage = np.int16(rImage)
    hdu = fits.PrimaryHDU(intImage)
    fname = outdir + 'python_lucky/' + 'li_NEW_' + strIndex + '.fits'
    fits.writeto(fname, intImage, header)
